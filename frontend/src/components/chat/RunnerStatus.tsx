...
function formatRelative(value: string | number | null | undefined, t: Translate): string {
  if (value == null || value === "") return t("runner.never");
  const then = typeof value === "number"
    ? (value < 1_000_000_000_000 ? value * 1000 : value)
    : new Date(value).getTime();
  if (!Number.isFinite(then)) return t("runner.unknown");
  const deltaSec = Math.round((Date.now() - then) / 1000);
  if (deltaSec < 0) return t("runner.justNow");
  if (deltaSec < 60) return t("runner.agoSeconds", { count: deltaSec });
  if (deltaSec < 3600) return t("runner.agoMinutes", { count: Math.floor(deltaSec / 60) });
  if (deltaSec < 86_400) return t("runner.agoHours", { count: Math.floor(deltaSec / 3600) });
  return t("runner.agoDays", { count: Math.floor(deltaSec / 86_400) });
}
...
function summarizeLimits(limits: LiveMandateLimits | undefined, t: Translate): string {
  if (!limits) return "";
  const parts: string[] = [];
  ...
  if (limits.max_leverage != null) parts.push(limits.max_leverage <= 1 ? t("runner.noLeverage") : `${limits.max_leverage}×`);
  return parts.join(" · ");
}
...
function BrokerRow({
  ...
  const authorizeInstruction = authorizeHint?.instruction
    ?? (authorizeFailed ? t("runner.authorizeFallback") : t("runner.authorizeLoading"));
  const authorizeNote = t("runner.authorizeNote");
  ...
  const toggleRunner = useCallback(async () => {
    ...
    try {
      if (runnerAlive) {
        ...
        toast.success(t("runner.runnerStopped", { broker: brokerKey }));
      } else {
        await api.startLiveRunner(brokerKey);
        toast.success(t("runner.runnerStarted", { broker: brokerKey }));
      }
      onRefresh();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("runner.runnerControlFailed"));
    } finally {
      setBusy(false);
    }
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-md border bg-background/60 p-2">
              ...
              <div className={["mt-0.5 text-xs font-semibold", runnerAlive ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"].join(" ")}>
                {runnerAlive ? t("runner.running") : t("runner.stopped")}
              </div>
            </div>
            <div className="rounded-md border bg-background/60 p-2">
              <div className="flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                {mandate.expires_at && (
                  <span
                    ...
                    title={t("runner.expiresTitle", { time: new Date(mandate.expires_at).toLocaleString() })}
                  >
                    <Clock className="h-2.5 w-2.5" />
                    {countdown.expired ? t("runner.expired") : t("runner.expiresIn", { time: countdown.label })}
                  </span>
                )}
              </div>
              <div className="mt-0.5 font-mono text-[11px] text-foreground">
                {summarizeLimits(mandate.limits, t) || t("runner.limitsUnavailable")}
              </div>
            </div>
          ) : (
            <div className="rounded-md border border-dashed bg-background/40 p-2 text-[10px] text-muted-foreground">
              {t("runner.noActiveMandate")}
            </div>
          )}
          ...
          <div className="flex items-center justify-between gap-2">
            {halted ? (
              <span className="inline-flex items-center gap-1 text-[10px] font-medium text-destructive">
                ...
                {t("runner.haltedControlsDisabled")}
              </span>
            ) : (
              <span className="text-[10px] text-muted-foreground">
                {runnerAlive ? t("runner.runtimeActive") : t("runner.idle")}
              </span>
            )}
            <button
              type="button"
              ...
              title={runnerAlive ? t("runner.stopRunnerTitle") : t("runner.startRunnerTitle")}
            >
              {busy ? <Loader2 className="h-3 w-3 animate-spin" /> : <Power className="h-3 w-3" />}
              {runnerAlive ? t("runner.stopRunner") : t("runner.startRunner")}
            </button>
          </div>
        </>
      )}
  ...
  return (
    <div className="grid gap-2">
      <button
        ...
        aria-label={t("runner.status")}
        aria-expanded={open}
      >
        <Activity className="h-3 w-3 shrink-0" />
        <span className="shrink-0">{t("runner.status")}</span>
        <span className="truncate text-muted-foreground">
          {authorizedCount > 0 ? t("runner.connectedCount", { count: authorizedCount }) : t("runner.noConnectorConnected")}
        </span>
        {anyRunning && !isHalted && (
          <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-1.5 py-0.5 text-[10px] font-medium text-emerald-600 dark:text-emerald-400">
