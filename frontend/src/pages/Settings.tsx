...
export function Settings() {
    ...
    Promise.all([api.getLLMSettings(), api.getDataSourceSettings()])
      ...
      .catch((error) => {
        ...
        if (isAuthRequiredError(error)) {
          ...
          toast.error(t("settings.loadLlmFailed", { message }));
          toast.error(t("settings.loadDataFailed", { message }));
        }
      })
      .finally(() => {
  ...
  const submitLocalApiKey = (event: FormEvent) => {
    ...
    toast.success(t("settings.localApi.saved"));
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setApiKey("");
      setClearApiKey(false);
      toast.success(t("settings.saveLlmOk"));
    } catch (error) {
      toast.error(t("settings.saveLlmFailed", { message: error instanceof Error ? error.message : "Unknown error" }));
    } finally {
      setSaving(false);
    }
  };
  ...
  const submitDataSources = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.success(t("settings.saveDataOk"));
    } catch (error) {
      toast.error(t("settings.saveDataFailed", { message: error instanceof Error ? error.message : "Unknown error" }));
    } finally {
      setDataSaving(false);
    }
  };
    ...
    <form onSubmit={submitLocalApiKey} className="rounded-lg border bg-card p-5 shadow-sm">
      <div className="mb-4 space-y-1">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold">{t("settings.localApi.title")}</h2>
        </div>
        <p className="text-sm text-muted-foreground">{t("settings.localApi.desc")}</p>
      </div>
      <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
        <label className="grid gap-2">
          <span className={labelClass}>{t("settings.localApi.key")}</span>
          <input
            type="password"
            value={localApiKey}
            onChange={(event) => setLocalApiKeyState(event.target.value)}
            className={fieldClass}
            placeholder={t("settings.localApi.placeholder")}
        ...
        >
          ...
          {t("settings.localApi.save")}
        </button>
      </div>
      <p className="mt-2 text-xs text-muted-foreground">{t("settings.localApi.placeholder")}</p>
    </form>
  );
  ...
  const keyStatus = settings.api_key_configured
    ...
    : settings.api_key_required
      ...
      : selectedProvider?.auth_type === "oauth" && selectedProvider.login_command
        ? t("settings.oauthCommand", { command: selectedProvider.login_command })
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">{t("settings.llm.desc")}</p>
      </div>
        ...
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{t("settings.providerHint")}</span>
            </label>
            ...
            <label className="grid gap-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  title={t("settings.providerDefaults")}
                >
                  <RotateCcw className="h-4 w-4" />
                  <span className="hidden sm:inline">{t("settings.providerDefaults")}</span>
                </button>
              </div>
              <span className={hintClass}>{t("settings.modelHint")}</span>
            </label>
            ...
            <label className="grid gap-2">
              <span className={labelClass}>
                {selectedProvider?.auth_type === "oauth" ? t("settings.oauth") : t("settings.apiKey")}
              </span>
        ...
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{t("settings.reasoningHint")}</span>
            </label>

            <div className="rounded-md border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
              <span className="font-medium text-foreground">{t("settings.savedTo")}: </span>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <div className="flex items-center justify-between gap-3">
                <span className={hintClass}>{t("settings.tushareHint")}</span>
                <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
            ...
            >
              ...
              {dataSaving ? t("settings.saving") : t("settings.saveData")}
            </button>
          </div>
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              ...
              <p>
                {dataSettings.baostock_installed
                  ? t("settings.pythonInstalled")
                  : t("settings.pythonNotInstalled")}
              </p>
            </div>
          </div>
        </div>
